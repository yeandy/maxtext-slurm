...
3. **Check whether training was progressing or stalled before the heartbeat error.** A heartbeat timeout can be (a) the sole issue (pure gRPC false positive — training was actively progressing when killed), (b) secondary to an RCCL hang (training stalled, then gRPC deadlocked), or (c) secondary to another failure that killed the accused task without leaving a log trace (e.g., OOM-kill via SIGKILL, node preemption). Item 2 above catches case (c) when log evidence exists; this item catches cases (a) and (b), and also case (c) when there's no log evidence but TSDB reveals the issue.
   - **With TSDB** (`RAY=1` job with `prometheus/` directory): query GPU power and utilization in the period **before** the heartbeat error (e.g., 30-60 minutes before). If GPUs show normal training power (~800-1000W) right up to the heartbeat error, the heartbeat timeout is the sole issue — a pure gRPC false positive with no preceding hang or failure. If GPUs show 100% utilization + idle-level power (~260-310W) before the heartbeat error, the primary failure is an **RCCL hang**. If GPUs show 0% utilization + idle power on specific nodes while others are active, those nodes likely died (OOM-kill, hardware fault) — investigate those nodes. Also check `hw_oom_kills_total`, `hw_dmesg_gpu_errors_total`, and memory pressure metrics to identify non-hang root causes.
   - **Without TSDB**: compare `sum(all step seconds)` against wall time (see workflow step 5). If the gap is small (within expected setup/compilation overhead of ~15-30 min), there was likely no preceding stall — the heartbeat timeout is the sole issue. A significant gap signals unaccounted time that could be a hang, slow checkpoints, or other stalls — cross-reference with other failure signatures in the log.
