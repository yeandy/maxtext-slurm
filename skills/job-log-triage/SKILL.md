...
4. **Query the Prometheus TSDB** for GPU utilization and power. The definitive RCCL hang signature is:
   - `ray_node_gpus_utilization` = **100%** (GPUs busy-waiting in RCCL polling loop)
   - `hw_gpu_power_watts` = **idle-level** (~300W on MI300X, vs 600-700W+ during active training)
   ...
   - Also check `hw_tcp_retransmits_total` and RDMA counters around the hang time to identify whether a network event triggered the deadlock.
