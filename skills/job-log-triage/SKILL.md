...
4. **Query the Prometheus TSDB** for GPU utilization and power. The definitive RCCL hang signature is:
   ...
   - `hw_gpu_power_watts` = **idle-level** (~260-310W on MI300X/MI355X, vs 600-700W+ during active training)
   ...
   - **TCP retransmit analysis:** Check `hw_tcp_retransmits_total` in two ways: (1) **rate during hang** — `increase(hw_tcp_retransmits_total[<hang_duration>])` to detect active network issues; (2) **absolute totals across nodes** — nodes with totals orders of magnitude higher than peers (e.g., 10M vs 10K) have chronically degraded network hardware (bad NICs, cables, or switch ports) and are prime suspects for triggering the deadlock even if their current retransmit rate is low. Recommend excluding such nodes on resubmit.
